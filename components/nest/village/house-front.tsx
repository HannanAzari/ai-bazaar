...
import { ChevronLeft, ChevronRight, DoorOpen, MapPin, Sparkles } from "lucide-react";
...
// M19 — the House Front: what you see before you enter a Nest. Think Animal Crossing
// meets Linktree — you arrive at a *place* with a face, a name, a door. Reused by the
// village arrival overlay and the /@handle direct-arrival hero.
...
export function HouseFront({
  ...
  house: House;
  /** Back to the village (overlay close or /village link). */
  onBack?: () => void;
  onPrev?: () => void;
  onNext?: () => void;
  backLabel?: string;
  ...
  return (
    <div className={`relative overflow-hidden ${className}`}>
      ...
      {/* top bar — back + presence */}
      <div className="relative z-10 flex items-center justify-between p-3">
        {onBack ? (
          <button onClick={onBack} className="inline-flex items-center gap-1 rounded-full bg-white/80 px-3 py-1.5 text-xs font-bold text-ink/70 shadow-soft backdrop-blur active:scale-95">
            <ChevronLeft className="size-4" /> {backLabel}
          </button>
        ) : <span />}
        {house.online ? (
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white/80 px-3 py-1.5 text-xs font-bold text-meadow-shade shadow-soft backdrop-blur">
            <span className="size-2 rounded-full bg-meadow-shade" /> Home
          </span>
        ) : (
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white/70 px-3 py-1.5 text-xs font-bold text-ink/40 shadow-soft backdrop-blur">
      ...
      <div className="relative z-10 flex flex-col items-center px-5">
        <div className="nest-approach relative w-full max-w-[300px]">
          <HouseExterior house={house} className="w-full" />
          {/* neighbor arrows flank the house */}
        ...
        {house.latestNestTitle ? (
          <p className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-white/70 px-3 py-1 text-xs font-bold text-ink/55 backdrop-blur">
            <Sparkles className="size-3 text-terracotta" /> Latest: {house.latestNestTitle}
          </p>
        ) : null}
        ...
        <div className="mt-5 w-full max-w-xs">
          {canEnter ? (
            <button onClick={enter} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-terracotta px-5 py-3.5 text-base font-black text-parchment shadow-lift transition active:scale-95">
              <DoorOpen className="size-5" /> Enter Nest
            </button>
          ) : (
            <div className="rounded-2xl border border-dashed border-timber/30 bg-white/60 px-5 py-3.5 text-center text-sm text-ink/50">
              {house.isReal ? "This creator hasn't opened their Nest yet." : "Nobody's moved in here yet — this lot is still empty."}
            </div>
          )}
        </div>
      </div>
      ...
      {entering && house.nestHref ? (
        <DoorTransition style={house.style} onDone={() => router.push(house.nestHref!)} />
      ) : null}
    </div>
  );
}
