...
| `/village` | — | **M19 Village + M19.1 atmosphere + Beta Polish 2 realism** — a believable town on a **`VillageTerrain`** ground (rolling grass valley, elevation, **winding road** + walking paths, scattered trees/bushes/flowers/rocks) under a live time-of-day + weather sky; houses have **perspective** (lower closer/bigger, upper further/smaller) + **contact shadows** + seed variety (fences/porches/roofs); tap a house → camera zooms → **arrival** (Beta Polish 3: Creator · Followers · Nest count · Enter — no online/presence/now-showing clutter; house feels alive; **swipe left/right** like Instagram Stories + prev/next arrows + back); **Enter → door → Nest**; **Exit → door → back**. Navigation unchanged from M19 |
...
| `/home` (Beta Polish 1 + 4) | — | **True vertical paging** — one Nest per viewport, swipe to page, **no peek**; **premium cards** (serif title, ringed avatar, CTA hierarchy, vignette), furniture stays clear of the buttons (safe area), smooth swipe + shimmer loading / image fade-in |
