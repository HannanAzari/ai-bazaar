...
| M36 — Identity Lock Pipeline **(HELD, not pushed)** | 2026-07-16 | Reverse the priority to **IDENTITY > FUNCTION > DNA**: a real object must stay recognisably itself. New pipeline `lib/identity/`: photo → segmentation → **identity contract** (structured extraction) → single generation (cutout + original + mask + identity-constrained prompt) → **identity validator (Gate 1, hard)** → **targeted repair** (re-apply the source's own critical regions; never rebuild) → style validator (Gate 2). Enforce identity in post — never trust the model. Route + Gemini send original+mask+cutout. Model benchmark **investigation only** ([design/IDENTITY_MODEL_BENCHMARK.md](design/IDENTITY_MODEL_BENCHMARK.md)), no model change. **Mug test does NOT yet fully pass** (black handle preserved; cream/lettering/style not clean) → **local commit `4682bcb`, HELD, not pushed** per instruction. Gates green (538 tests). See [design/IDENTITY_LOCK.md](design/IDENTITY_LOCK.md). |
| M35 — The Nestudio Art Engine | 2026-07-16 | Make AI assets share **one artistic language** with the official library — by MEASURING it. `lib/art-engine/`: fingerprint the real golden-nest assets → mean+variance **StyleProfile** (measuring corrected wrong assumptions: officials are crisp-edged, tightly cropped, matte ≈ 0.60). **Style Validator** gate (~7/8 officials belong; palette discipline the one hard veto). **Conform** pass pulls output into the material language identity-preserving. `/dev/art-engine` (family test + 9 benchmark objects). A generated mug clears the gate at ~63% ("belongs"). Commit `02e75e2` (**pushed HEAD**). Gates green (533 tests). See [design/NESTUDIO_DNA_MEASURED.md](design/NESTUDIO_DNA_MEASURED.md). |
| M34 — Premium Segmentation Experience | 2026-07-16 | Replace the manual cutout editor with real **on-device object selection** (Telegram/Apple-Photos feel). `lib/segmentation/`: MediaPipe Interactive Segmenter (`magic_touch`, bundled) + classical flood fallback (never fails) — **tap the object, background disappears (~237ms, <300ms target)**, all in-browser (no server roundtrip, not Gemini). Auto-detect on open; brush is now a "Fix edges" fallback. Warm paper card (no giant checkerboard). Commit `ebd97d5` (pushed). Gates green (523 tests). |
| M33 — Asset Creation Experience Polish | 2026-07-15 | Make asset creation *feel* premium — forget you're using AI. No AI/generation changes; UX/interaction/motion only. **Original photo first** → **auto-cutout first** (brush is a fallback) → **single generation** (not 3) + **refinement loop** (Use/Improve → "what to change?" → regenerate one) → premium **glass modal** + staged loading + **success moment** → reveal in **My Assets** (scroll+pulse). Library tabs Official/My Assets/Recent. Commit `84b5569` (pushed). Gates green (517 tests). |
