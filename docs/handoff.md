 ...
 **Production Cutover V1** (2026-06-23): real Supabase **auth** (unified `useSession()`), **profiles**, and **room persistence** (jsonb on `rooms`), `SupabaseStorage`, `/onboarding`, and subdomain prep — all mode-selected, **demo unchanged & default**. Live-verified: demo full flow + production auth rejecting bad creds. DB persistence/RLS deferred to `docs/staging-checklist.md` (schema not applied; anon-only).
- **Gates (all green):** `npm run typecheck && npm run lint && npm run test && npm run build` (168 tests, ~82 pages).
