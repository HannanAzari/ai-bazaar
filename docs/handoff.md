 ...
 **Production Cutover V1** (2026-06-23): real Supabase **auth** (unified `useSession()`), **profiles**, **room persistence** (jsonb on `rooms`), production **shop-claiming** (`lib/shop-claim.ts`: `claimShopInSupabase`/`getShopByAddress`), `SupabaseStorage`, `/onboarding`, subdomain prep — all mode-selected, **demo unchanged & default**. **Full authenticated flow verified LIVE on staging** (create account → onboarding → claim Nest → save → reload → persists from Supabase → logout → login → still exists; RLS owner-write/anon-deny/public-read hold). **Pilot Hardening V1** (2026-06-24): shared validation (`lib/validation.ts`), centralized friendly errors (`lib/errors.ts` — no raw Supabase leaks), loading/double-submit guards, internal funnel events, draft legal pages (`/privacy`,`/terms`,`/safety`,`/contact`); **safe for friends & family pilot** (see `docs/pilot-readiness.md`), not public-launch ready. **Analytics + Discovery V1** (2026-06-25, ADR-020): **mode-aware durable analytics** (`trackEvent` writes Supabase via the now-implemented `SupabaseEventsRepository` in production, localStorage in demo, local fallback on failure), **anonymous visitor sessions** (`lib/visitor-id.ts`/`lib/visitor-session.ts`: first/returning, start/end, duration), per-object `object_view` impressions, a pure **creator insights** engine (`lib/creator-insights.ts`) surfaced as a Studio **Insights & visitors** panel (visits, unique visitors, room entries, avg session, top objects/room/day, funnel + conversion), and **Featured Nests** discovery (`lib/discovery.ts` + `components/featured-nests.tsx`) on `/discover` (Trending/New/Recently active). `visitor_sessions` table + owner-read events RLS (`20260625_*`); demo unchanged. **Nestudio V2 editor line (M7A–M7C):** the front-facing Nest editor + Golden Living Nest, hotspots, and **structured in-Nest navigation** `Main Nest → Focus Area → Detail Scene → Back` (ADR-029). **M7C.1 — Hybrid Focus** (2026-06-30, ADR-030): a Focus Area resolves to a **Zoom Region** (true crop zoom of the *existing* scene) or a **Detail Surface** (authored close-up), with a pure **resolution audit** that measures whether existing artwork holds up before recommending any replacement (it does — no hi-res needed), crop-local child objects, and a progressive hi-res contract. Internal visitor demo: `/design/nest-hybrid-focus`. Master: [nest-hybrid-focus-v1.md](nest-hybrid-focus-v1.md). Additive + back-compat; **no new artwork, no Supabase**. **M7C.2 — Focus-first** (2026-06-30, refines ADR-030): *in Main view a Focus Area owns the first tap; the object's gallery/video/shelf interaction activates only after focus entry.* Persistent "Zoom to…" pills removed (whole trigger region is an accessible button + a single transient first-visit hint); trigger vs crop bounds separated and calibrated (TV cleared of avatar/plant + reliably reachable, Frame crop tightened); deterministic `recommendCrop`; rapid-tap/transition-state protection; Desk Surface kept explicitly provisional pending a dedicated background. **M7C.3 — Creator-authored crops + full-screen focus** (2026-06-30, refines ADR-030): *the creator visually authors and owns the final zoomed composition; the system only suggests.* The focused view now uses a **COVER** transform (`focusViewportTransform`) in a `fixed inset-0` full-width **3:4 cinematic hero** (scroll-locked, safe-area aware); a real **photo-crop editor** in the Nest editor (Tap area / Zoomed view toggle, Preview zoom, Use suggested crop) with `cropSource: suggested|creator_authored` so suggestions are never silently authoritative; recalibrated crops (TV + frame fill 100%×100%, bookshelf excludes the desk); crop persists through save/load/export/import + undo/redo. **M7C.4 — V1 fixed-ratio + in-place cinematic** (2026-06-30, refines ADR-029): *one creator-authored rectangle (`focusBounds`, the Nest aspect ratio) is BOTH tap target and cinematic destination; the existing scene transforms in place — no modal, no backdrop, no duplicate stage.* The trigger/crop split + cover transform are retired (legacy migrates via `focusBoundsOf`/`normalizeLegacyFocusArea`). Fixed-ratio geometry (`fitRectToAspectRatio`/`moveRectInsideBounds`/`resizeRectWithLockedAspect`) + `cinematicFocusTransform`. The **Focus editor input-blocking bug is fixed** (overlay host is now `pointer-events-none`; one fixed-ratio rectangle; sheet = Add · Preview focus · Reset · Delete). Visitor + editor "Preview focus" share `CinematicFocusStage`. **M7C.5 — integration repair + minimal creator UX** (2026-06-30): documented editor **z-index layer model** ([`lib/nest-editor-layers.ts`](../lib/nest-editor-layers.ts)) so Focus overlays/handles never paint over the bottom drawer; the editor **Preview now renders `NestSceneNavigator`** (not a static stage) so authored Focus Areas work like the visitor route (shared navigator + `autoEnterFocusId` for the "Preview focus" shortcut); the Focus sheet is **compact** (chips · Add · inline rename · Preview · Delete) with Enabled/Lock/Reset/hint/advanced moved to Template/Internal-only. **M7C.6 — Focus Areas are entrances to nested editable scenes** (2026-07-01, ADR-031): a Focus Area opens an **editable child scene** (`sceneType:"focus"`, `parent_crop` base) with its own objects/hotspots; the editor **"Enter area"** opens it in Edit mode (Arrange/Assets/Connect on scene-local objects), Back returns to the parent editor; the visitor enters the same child scene (parent-crop base + child objects). **Fixed the release-blocker** where editor-authored areas (no `zoomRegion`) were hidden from Preview/visitor + rejected by validation. Scene graph + pure scene stack (`MAX_FOCUS_DEPTH=3`) built over the existing `detailScenes` storage; full save/load/export/import + undo/redo. **M7C.7 — child visual base repair** (2026-07-01): the child editor renders the **transformed parent crop** (shared `FocusedParentBase`, identical during/after the animation and in the editor) instead of the flat empty-room background, and the transparent overlay clears its gradient **background-image** so the crop never disappears (`resolveFocusSceneBase`, or an explicit error — never a blank room). **M7C.8 — inherited interactions + projection** (2026-07-01, additive design note, no new ADR): parent objects intersecting a Focus Area become read-only **interaction proxies** inside the child scene (Strategy A — no duplicated art) with per-hotspot child **binding overrides** (child beats parent, else inherit); native child objects **project back into Main** clipped to the Focus Area (focus-first; default-on `projection` policy). One shared transform (`lib/nest-focus-projection.ts`) for render/hit-test/tests; one nesting level; no persisted duplicates.
- **Gates (all green):** `npm run typecheck && npm run lint && npm run test && npm run build` (561 tests). *Local dev note:* `next dev` in the sandbox mis-registers a subset of `app/design/*` routes (fsevents/watcher quirk affecting pre-existing routes too); verify internal `/design/*` surfaces via `npm run build && npm run start`.
