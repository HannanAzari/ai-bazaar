...
- **Gates (all green):** `npm run typecheck && npm run lint && npm run test && npm run build` (125 tests, ~81 pages).
...
- **localStorage keys** (`ai-bazaar-*`, e.g. `ai-bazaar-rooms`, `ai-bazaar-shop`, `ai-bazaar-design-drafts`) — renaming silently wipes demo state.
